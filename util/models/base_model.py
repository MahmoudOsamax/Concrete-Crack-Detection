...
class BaseModel(ABC):
    ...
    def optimize_parameters(self):
        pass
    def setup(self, opt):
        """Load networks and create schedulers."""
        try:
            if self.isTrain:
    ...
    def save_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                logger.info(f"Saved network {name} to {save_path}")
    def __patch_instance_norm_state_dict(self, state_dict, module, keys, i=0):
        """Fix InstanceNorm checkpoints incompatibility."""
        key = keys[i]
        if i + 1 == len(keys):
            if module.__class__.__name__.startswith('InstanceNorm') and \
                    (key == 'running_mean' or key == 'running_var' or key == 'num_batches_tracked'):
    ...
    def print_networks(self, verbose):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if verbose:
                    logger.info(str(net))
                logger.info(f'[Network {name}] Total number of parameters : {num_params / 1e6:.3f} M')
        logger.info('-----------------------------------------------')
